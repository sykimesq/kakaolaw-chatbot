...
- 격락손해는 '차량 가치 하락'이므로, 수리 명세서의 수리 항목이 핵심이다.
- 반드시 물어볼 것:
  · 수리 명세서를 받았는가? (사진/서류 확인)
  · 명세서에 '쿼터패널(사이드패널/판금) 수리' 항목이 있는가?
    → 쿼터패널 판금수리는 격락손해 청구 대상, 일반 도어/패널은 대상 아님
  · 차량 출고일/연식 (격락손해는 대체로 출고 후 얼마 안 된 차만 인정)
  · 총 수리비용과 차량 현재 시세(수리비가 차값의 일정 비율 넘는지)
  · 상대방 보험사가 거절한 사유(어떤 근거로 안 된다는지)
- 질문 예시: "수리 명세서에 쿼터패널(사이드패널) 판금 항목이 포함되어 있나요?"
